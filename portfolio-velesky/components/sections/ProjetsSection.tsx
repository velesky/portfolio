...
export function ProjetsSection() {
  ...
  const updateCardDOM = useCallback((index: number) => {
    ...
    if (badgesEl) {
      badgesEl.innerHTML = projet.stack
        .map(
          (s) =>
            `<span style="display:inline-block;font-size:0.75rem;padding:2px 8px;border-radius:9999px;background:rgba(200,240,74,0.08);color:#c8f04a;border:1px solid rgba(200,240,74,0.2);font-family:DM Mono,monospace">${s}</span>`
        )
        .join("");
    }
    ...
    if (linksEl) {
      ...
      if (projet.github)
        links.push(
          `<a href="${projet.github}" target="_blank" rel="noopener noreferrer" style="font-size:0.75rem;color:#6b6b6b;font-family:DM Mono,monospace">↗ GitHub</a>`
        );
      if (projet.lien)
        links.push(
          `<a href="${projet.lien}"   target="_blank" rel="noopener noreferrer" style="font-size:0.75rem;color:#6b6b6b;font-family:DM Mono,monospace">↗ Voir le projet</a>`
        );
  ...
  useEffect(() => {
    const init = () => {
      ...
      if (isMobile || !zone || !card || !start || !end) return;
  ...
  }, [isMobile, swapContent]);

  // ── Rendu ─────────────────────────────────────────────────────────────────
  return (
    <section id="projets" ref={sectionRef} className="relative py-24 md:py-32 px-6 md:px-10">
      <div className="max-w-6xl mx-auto">
        ...
        {/* ── Zone principale MotionPath (Desktop/Tablet) ───────────────────────────────────── */}
        {!isMobile ? (
          <div
            ref={zoneRef}
          ...
          >
            ...
            <div
              ref={endMarkerRef}
              className="absolute"
              style={{ bottom: "2%", left: 0, width: "1px", height: "1px" }}
            />

            {/* Card mobile (GSAP animated) */}
            <div
              ref={cardRef}
            ...
            >
              <div
                ...
                style={{
                  background:
                    "radial-gradient(ellipse at top left, rgba(200, 240, 74, 0.05) 0%, transparent 65%)",
                }}
              />
          ...
          /* ── Version Mobile (Liste standard épurée) ────────────────────────────────────────── */
          <div className="flex flex-col gap-12 mt-12">
            {projets.map((projet, i) => (
              <RevealAuScroll key={projet.id} delay={i * 0.1}>
                <div
                  className="group relative rounded-3xl overflow-hidden border border-white/5 bg-[#0f0f0f] p-6"
                  style={{
                    background:
                      "radial-gradient(circle at top left, rgba(200, 240, 74, 0.03), transparent)",
                  }}
                >
                  <div
                    className="absolute top-4 right-6 font-bebas text-6xl opacity-[0.03] select-none pointer-events-none"
                    style={{ color: ACCENT }}
                    aria-hidden="true"
                  >
                  ...
                  <div className="flex flex-col gap-4">
                    <h3 className="font-bebas text-3xl tracking-wide text-[#e8e4dc]">
                      {projet.titre}
                    </h3>

                    <p className="font-dm-mono text-sm leading-relaxed text-neutral-500">
                      {projet.description}
                    </p>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      {projet.stack.map((s) => (
                        <span
                          key={s}
                          className="font-dm-mono text-[10px] px-2 py-1 rounded-full bg-white/5 text-accent/80 border border-accent/10"
                        >
                          {s}
                        </span>
                      ))}
                    ...
                    <div className="flex items-center gap-6 pt-4 mt-2 border-t border-white/5">
                      {projet.github && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="font-dm-mono text-xs text-neutral-400 hover:text-accent transition-colors flex items-center gap-1"
                        >
                          ↗ GitHub
                        </a>
                      )}
